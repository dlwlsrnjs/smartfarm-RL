    ...
    def _on_step(self) -> bool:
        ...
        if self.n_calls % self.eval_freq == 0:
            ...
            if self.verbose >= 1:
                ...
                try:
                    # Friendly Korean debug summary
                    print("[평가] 진행상황 요약 →",
                          f"스텝 {self.num_timesteps}",
                          f"에피소드 평균보상 {mean_reward:.2f}",
                          f"평균 수익합 {float(np.mean(sum_profits)):.2f}",
                          f"평균 위반합(온도/CO2/RH) {float(np.mean(sum_violations[:,0])):.3f}/"
                          f"{float(np.mean(sum_violations[:,1])):.3f}/"
                          f"{float(np.mean(sum_violations[:,2])):.3f}")
                except Exception:
                    pass
            ...
            if mean_reward > self.best_mean_reward:
                ...
                try:
                    print("[평가] 새 최고 성능 갱신! 평균보상:", f"{mean_reward:.2f}")
                except Exception:
                    pass
